...
async function crawlNaverCafe(cafeInfo: CafeInfo, limit = 5) {
...
async function saveToSupabase(supabase: any, posts: any[]) {
  try {
    ...
    const existingUrlSet = new Set(existing?.map((e: any) => e.original_url) || []);
    ...
    if (makeWebhookUrl && data.length > 0) {
      try {
  ...
  try {
    ...
    for (const cafeInfo of TARGET_CAFES) {
      ...
      try {
        ...
        const saved = await saveToSupabase(supabase, posts);
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `총 ${allResults.length}개의 게시글이 처리되었습니다.`,
        data: allResults
      }),
      {
